...
const advanceFormation = (
  state: { weights: number[]; velocity: number[] },
  flow: number,
  elapsed: number,
  duration: number,
  frozen: boolean
...
const mixColor = (from: number[], to: number[], amount: number): [number, number, number, number] => [
...
const parseColor = (value: string, fallback: string): [number, number, number, number] => {
...
const resolveQuality = (canvas: HTMLCanvasElement) => {
  const memory = (navigator as any).deviceMemory || 6;
...
const resolveDpr = (preset: { dpr: number; supersamplePixels: number }, canvas: HTMLCanvasElement) => {
  const cssPixels = Math.max(1, canvas.clientWidth * canvas.clientHeight);
...
const resolveBloomSize = (size: number[], qualityLevel = 0): [number, number] => {
...
const createRenderGraph = (gpu: any, outputSize: number[], bloomSize = resolveBloomSize(outputSize)) => {
...
const configureStyle = (graph: any, settings: any, outputSize: number[], cssSize: number[]) => {
...
export default function AeroShards({
  ...
  const resolvedMaterial = (MATERIAL_PRESETS as any)[material] || MATERIAL_PRESETS.pearl;
  const resolvedDetail = (DETAIL_PRESETS as any)[detail] || DETAIL_PRESETS.balanced;
  ...
  useEffect(() => {
    ...
    // Check WebGPU availability gracefully
    if (typeof navigator === 'undefined' || !(navigator as any).gpu) {
      setHasError(true);
      onErrorRef.current?.(new Error("WebGPU is not supported or enabled in this browser."));
      return;
    }
