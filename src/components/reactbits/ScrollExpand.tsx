...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      track.style.height = `${stageH * (1 + Math.max(0.1, c.scrollDistance) + Math.max(0.1, c.holdDistance))}px`;

      const w = window.innerWidth || root.clientWidth || stageH;
      stage.style.setProperty('--se-title-size', `${clamp(w * 0.055, 24, 72)}px`);
    };
    ...
    // Initial delayed re-measurements to account for fonts, styles, and image loading
    const timer1 = setTimeout(onResize, 50);
    const timer2 = setTimeout(onResize, 300);
    const timer3 = setTimeout(onResize, 800);
  ...
  const handleMediaLoad = () => {
    ...
    const c = propsRef.current;
    const stageH = c.useWindowScroll ? window.innerHeight : rootRef.current?.clientHeight || 800;
    stage.style.height = `${stageH}px`;
    track.style.height = `${stageH * (1 + Math.max(0.1, c.scrollDistance) + Math.max(0.1, c.holdDistance))}px`;
  };
  ...
  return (
    ...
    >
      <div ref={trackRef} className="scroll-expand__track">
        <div
          ref={stageRef}
          className="scroll-expand__stage"
